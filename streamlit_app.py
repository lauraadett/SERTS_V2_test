...
price_hp = st.sidebar.slider(
    ...
    min_value=0.05, max_value=0.90, value=0.38, step=0.01, format="%.2f"
)
...
installation = st.sidebar.slider(
    ...
    min_value=500, max_value=16000, value=6500, step=100,
)
...
def calculate_lcoh_scenarios(params):
    ...
    npv_heat_hp = sum(q_heat / ((1 + r) ** t) for t in range(1, n_hp + 1))
    npv_heat_gb = sum(q_heat / ((1 + r) ** t) for t in range(1, n_gb + 1))

    # Gas Boiler (discounted over its own lifetime n_gb)
    ...
    lcoh_gb = npv_cost_gb / npv_heat_gb
    ...
    for sc in SIZE_CLASSES:
        ...
        for T_supply in SUPPLY_TEMPS:
            ...
            lcoh_hp = npv_cost_hp / npv_heat_hp
            results[(sc, T_supply)] = {
                "Heat Pump":       lcoh_hp,
                "Gas Boiler":      lcoh_gb,
...
with col_cum:
    ...
    # Selected size class: plot W35, W45, W55
    for T_supply in SUPPLY_TEMPS:
    ...
    # Break-even annotations (staggered vertically to avoid overlap)
    # Detects BOTH directions: HP going cheaper than GB, or HP going costlier than GB
    v_offsets = {35: 2, 45: 5, 55: 8}
    for T_supply in SUPPLY_TEMPS:
        years_be, cum_hp_be, cum_gb_be = calculate_cumulative_costs(parameters, size_class, T_supply)
        for i in range(1, len(years_be)):
            hp_above_prev = cum_hp_be[i-1] > cum_gb_be[i-1]
            hp_above_curr = cum_hp_be[i]   > cum_gb_be[i]
            if hp_above_prev == hp_above_curr:
                continue  # no crossover at this step
            frac_denom = (cum_hp_be[i] - cum_hp_be[i-1]) - (cum_gb_be[i] - cum_gb_be[i-1])
            if frac_denom == 0:
                break
            frac = (cum_gb_be[i-1] - cum_hp_be[i-1]) / frac_denom
            be_year = (i - 1) + frac
            be_cost = (cum_hp_be[i-1] + frac * (cum_hp_be[i] - cum_hp_be[i-1])) / 1000
            # Label depends on which direction HP crosses GB
            if hp_above_prev:
                label = f"Break-even W{T_supply}: yr {be_year:.1f}"
            else:
                label = f"HP > GB W{T_supply}: yr {be_year:.1f}"
            ax_cum.axvline(x=be_year, color=colors_supply[T_supply],
                           linestyle=":", linewidth=1.5, alpha=0.8)
            # Flip text to the left if break-even is close to the end of x-axis
            x_offset = -3.5 if be_year > 15 else 1.0
            ax_cum.annotate(
                label,
                xy=(be_year, be_cost),
                xytext=(be_year + x_offset, be_cost + v_offsets[T_supply]),
                fontsize=9, color=colors_supply[T_supply], fontweight="bold",
                arrowprops=dict(arrowstyle="->", color=colors_supply[T_supply], lw=1.5),
                bbox=dict(boxstyle="round,pad=0.3", fc="white", ec=colors_supply[T_supply], alpha=0.9)
            )
            break
    ...
    ax_cum.legend(fontsize=10, frameon=False, loc="upper left")
...
""", unsafe_allow_html=True)    min_value=0.0, max_value=0.10, value=0.05, step=0.01, key="discount_rate_slider"
)
